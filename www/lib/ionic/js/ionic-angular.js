 ...
 * @module ionic
 * @description
 * $ionicHistory keeps track of views as the user navigates through an module. Similar to the way a
 * browser behaves, an Ionic module is able to keep track of the previous view, the current view, and
...
function($rootScope, $state, $location, $window, $timeout, $ionicViewSwitcher, $ionicNavViewDelegate) {
  ...
  return {
     ...
     * @description The module's view history data, such as all the views and histories, along
     ...
     * @description The module's current view.
     ...
     * @description Navigates the module to the back view, if a back view exists.
     */
     ...
     * @description Clears out the module's entire history, except for the current view.
     */
...
function($rootScope, $state, $location, $document, $ionicPlatform, $ionicHistory, IONIC_BACK_PRIORITY) {
  ...
  function onHardwareBackButton(e) {
    ...
    if (backView) {
      ...
      // there is no back view, so close the module instead
 ...
 * of your module. Additionally, `$ionicConfig` can also set and get config values during the run
 * phase and within the module itself.
 *
 ...
 * the module is running from. If the platform is not `ios` or `android` then it will default
 * to `ios`.
 ...
 * the module is running from. If the platform is `ios`, it will default to `center`.
...
/**
  ...
  *
  * * `platform`: Dynamically choose the correct title style depending on the platform
  * the module is running from. If the platform is `ios`, it will default to `left`.
 ...
 *
 * * `platform`: Dynamically choose the correct title style depending on the platform
 * the module is running from. If the platform is `ios`, it will default to `right`.
 * If the platform is `android`, it will default to `right`. If the platform is not
 ...
 * });
 * module.controller('AppCtrl', function($scope, $ionicLoading) {
...
function($rootScope, $ionicBody, $compile, $timeout, $ionicPlatform, $ionicTemplateLoader, $$q, $log, $ionicClickBlock, $window, IONIC_BACK_PRIORITY) {
  ...
  var ModalView = ionic.views.Modal.inherit({
    ...
    remove: function() {
      ...
      // However, this will make the module unresponsive for short amount of time.
 ...
 *
 * The Popover is a view that floats above an module’s content. Popovers provide an
...
.service('$ionicScrollDelegate', ionic.DelegateService([
   ...
   * @description Does not allow any of the module's scroll views to scroll either x or y.
   * @param {boolean=} shouldFreeze Should all module scrolls be prevented from scrolling or not.
   */
  'freezeAllScrolls',
  /**
   * @ngdoc method
 ...
 * dependency on the "ngIOS9UIWebViewPatch" module to your main module module.
 *
 * For example:
 *
 * ```
...
.controller('$ionicRefresher', [
  ...
  function($scope, $attrs, $element, $ionicBind, $timeout) {
    ...
    function setScrollLock(enabled) {
      ...
      // or remove it so the module can be natively scrolled
      if (enabled) {
        ionic.requestAnimationFrame(function() {
 ...
 * @description
 * `collection-repeat` allows an module to show huge lists of items much more performantly than
 * `ng-repeat`.
 *
...
* @restrict E
* @description
* The List is a widely used interface element in almost any mobile module, and can include
 ...
 * module will navigate back one view when the back button is clicked.  More advanced behavior is also
 ...
 *
 * @description
 * As a user navigates throughout your module, Ionic is able to keep track of their
 ...
 *
 * Ionic uses the AngularUI Router module so module interfaces can be organized
 ...
 *
 * @usage
 * In this example, we will create a navigation view that contains our different states for the module.
 *
 ...
 * });
 * ```
 * Then on module start, $stateProvider will look at the url, see if it matches the index state,
 * and then try to load home.html into the `<ion-nav-view>`.
 *
 * Pages are loaded by the URLs given. One simple way to create templates in Angular is to put
 * them directly into your HTML file and use the `<script type="text/ng-template">` syntax.
 * So here is one way to put home.html into our module:
 *
 * ```html
 ...
 * recreated, controllers are not loading again on a subsequent viewing. If the module/controller
 ...
 * Spinners enables you to give your users feedback that the module is
 ...
 * <table class="table spinner-table" ng-module="ionic">
 *  <tr>
 *    <th>
 *      <code>android</code>
 *    </th>
