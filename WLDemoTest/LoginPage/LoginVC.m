...
//    return;
    ...
    request.sendUser = ^(User *user){
        ...
        if(user.userId){
            NSLog(@"infomation is:%@",user);
            [UD setObject:user.username forKey:@"username"];
            [UD setObject:user.name forKey:FilePath];
            [UD synchronize];
            //[user save];
            CustomTabbar *tabbar = [[CustomTabbar alloc]init];
            UIWindow *window  = [UIApplication sharedApplication].keyWindow;
    ...
    [request loginRequestUserName:self.userNameTF.text psaaword:self.passwordTF.text withCtrl:self];
